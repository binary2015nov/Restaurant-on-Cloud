...
sql = f'''
SELECT A.ORDER_ID, A.ORDER_DETAILS_ID, DISH_NAME, ROUND(DISH_PRICE, 2) AS DISH_PRICE, ROUND(DISH_AMOUNT) AS DISH_AMOUNT, ORDER_TIME, DISH_PRICE * DISH_AMOUNT AS TOTAL_COST 
...
WHERE A.USERNAME = '{st.session_state.username}'
'''
...
for order_id, order_details in df_grouped:
    ...
    st.dataframe(order_details[['DISH_NAME', 'DISH_PRICE', 'DISH_AMOUNT', 'ORDER_TIME']], hide_index=True)
